...
const base = prod ? "/front_7th_chapter4-1/vanilla" : "";
...
if (prod) {
  ...
  app.use(base, (req, res, next) => {
    ...
    if (hasExtension) {
      express.static("dist/vanilla")(req, res, next);
    } else {
      next(); // SSR 라우트로 전달
    }
  });
...
const templatePath = prod ? path.resolve("dist/vanilla/index.html") : path.resolve("index.html");
...
app.use(async (req, res) => {
  try {
    ...
    if (prod) {
      ...
      const serverModulePath = path.resolve("./dist/vanilla-ssr/main-server.js");
      ...
      const serverModuleUrl =
        process.platform === "win32" && path.isAbsolute(serverModulePath)
          ...
          : "./dist/vanilla-ssr/main-server.js";
      const { render: renderFn } = await import(serverModuleUrl);
      render = renderFn;
    } else {
      // 개발: Vite를 통해 모듈 로드
      const { render: renderFn } = await vite.ssrLoadModule("/src/main-server.js");
      render = renderFn;
    }
