 ...
 */
export const buildHtmlContent = (template, pageData) => {
 ...
 */
export const createOutputPath = (route, distDir) => {
 ...
 * @param {string} distDir - 빌드 디렉토리 경로
 */
export const generatePage = async (route, renderFn, template, distDir) => {
 ...
 *
 * @param {string} originalUrl - 원본 URL (예: "/front_7th_chapter4-1/vanilla/?category=electronics")
 * @param {string} base - 기본 URL 경로 (예: "/front_7th_chapter4-1/vanilla/")
 * @returns {string} 정규화된 pathname (예: "/")
 */
export const extractPathname = (originalUrl, base) => {
  // base 경로 제거
