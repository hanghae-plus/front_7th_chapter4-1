...
function renderBreadcrumb({ category1, category2 }) {
...
function renderRating({ rating, reviewCount }) {
  ...
  return `
    <div class="flex items-center mb-3">
      ...
      <span class="ml-2 text-sm text-gray-600">${rating}.0 (${Number(reviewCount || 0).toLocaleString()}개 리뷰)</span>
    </div>
  `;
}
        ...
        <div class="grid grid-cols-2 gap-3 responsive-grid">
          ${relatedProducts
            ...
            <div class="bg-gray-50 rounded-lg p-3 related-product-card cursor-pointer"
              ...
              <p class="text-sm font-bold text-blue-600">${Number(p.lprice).toLocaleString()}원</p>
            </div>
          `,
            )
            .join("")}
...
function renderProductDetailBody({ product, relatedProducts = [] }) {
  if (!product) return "";
...
export function renderProductDetailPageView({ productState = {}, cart, ui }) {
  const { currentProduct: product, relatedProducts = [], error = null, loading = false } = productState;
  ...
  return PageWrapper({
    ...
    children: loading
      ...
      : error && !product
        ...
        : renderProductDetailBody({ product, relatedProducts }),
    cart,
    ui,
  });
}
