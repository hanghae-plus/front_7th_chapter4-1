...
export function SearchBar() {
  ...
  // 브레드크럼 카테고리 네비게이션
  const handleBreadCrumbClick = (e: MouseEvent<HTMLButtonElement>) => {
  ...
  // 1depth 카테고리 선택
  const handleMainCategoryClick = (e: MouseEvent<HTMLButtonElement>) => {
    const category1 = e.currentTarget.getAttribute("data-category1");
    if (!category1) return;
    setCategory({ category1, category2: "" });
  };
  ...
  // 2depth 카테고리 선택
  const handleSubCategoryClick = (e: MouseEvent<HTMLButtonElement>) => {
    const category1 = e.currentTarget.getAttribute("data-category1");
