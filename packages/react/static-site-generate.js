...
async function generateStaticSite() {
  ...
  const homeHtml = template
    ...
    .replace(
      `</body>`,
      homeResult.state
        ? `<script>window.__INITIAL_DATA__ = ${JSON.stringify(homeResult.state)};</script></body>`
        : `</body>`,
    );
    ...
    try {
      ...
      const html = template
        ...
        .replace(
          `</body>`,
          result.state
            ? `<script>window.__INITIAL_DATA__ = ${JSON.stringify(result.state)};</script></body>`
            : `</body>`,
        );
  ...
  const notFoundResult = await render("/존재하지않는경로");
  const notFoundHtml = template
    .replace(`<!--app-head-->`, notFoundResult.head ?? "")
    .replace(`<!--app-html-->`, notFoundResult.html ?? "")
    .replace(
      `</body>`,
      notFoundResult.state
        ? `<script>window.__INITIAL_DATA__ = ${JSON.stringify(notFoundResult.state)};</script></body>`
        : `</body>`,
    );
  ...
  console.log(`  총 ${items.length + 2}개 페이지 생성 (홈 + 상품 ${items.length}개 + 404)`);
}
