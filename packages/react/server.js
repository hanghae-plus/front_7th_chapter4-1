...
function normalizeUrl(originalUrl) {
  // 쿼리스트링 제거 후 base 제거
  let url = originalUrl.split("?")[0].replace(base, "");
  if (!url.startsWith("/")) {
    url = "/" + url;
  }
  return url;
...
async function createServer() {
  ...
  if (!prod) {
    ...
    app.use(`${base}assets`, sirv("./dist/react/assets", { maxAge: 31536000, immutable: true }));
